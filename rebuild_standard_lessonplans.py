...
def make_md_content(m):
    return f"""# RENCANA PEMBELAJARAN / LESSON PLAN
...
**Penyajian Ke-:** {m['no']:02d} dari 20 Pertemuan  
...
### C. PENYAJIAN MATERI (105 Menit)

| Sub Judul | Uraian Kegiatan Instruktur & Peserta | Media Pembelajaran | Waktu |
| :--- | :--- | :--- | :---: |
| **1. {m['sub1_judul']}** | {m['sub1_uraian'].replace('\n', '<br>')} | {m['sub1_media']} | {m['sub1_waktu']} |
| **2. {m['sub2_judul']}** | {m['sub2_uraian'].replace('\n', '<br>')} | {m['sub2_media']} | {m['sub2_waktu']} |
| **3. {m['sub3_judul']}** | {m['sub3_uraian'].replace('\n', '<br>')} | {m['sub3_media']} | {m['sub3_waktu']} |

---

### D. KESIMPULAN (5 Menit)
{m['kesimpulan']}  
*Media:* {m['kesimpulan_media']}

### E. APLIKASI (20 Menit)
{m['aplikasi']}  
*Media:* {m['aplikasi_media']}

### F. EVALUASI (30 Menit)
{m['evaluasi']}  
*Media:* {m['evaluasi_media']}

### G. TUGAS TAMBAHAN (5 Menit)
{m['tugas']}

### H. PEMBERESAN KELAS (5 Menit)
- Merapikan perangkat PC/HP dan ruang laboratorium (5R).
- Berdoa bersama dan memberikan salam penutup.
...
def generate_docx(m):
    ...
    docx_path = os.path.join(REPO_DIR, f"pertemuan-{m['no']:02d}.docx")
    doc.save(docx_path)
    
    md_path = os.path.join(REPO_DIR, f"pertemuan-{m['no']:02d}.md")
    with open(md_path, "w", encoding="utf-8") as f:\n        f.write(make_md_content(m))\n\n    print(f"Rebuilt Pertemuan {m['no']:02d} with master_lessonplan template.")
